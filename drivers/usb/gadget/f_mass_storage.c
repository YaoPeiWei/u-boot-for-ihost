...
reset:
	...
	#ifdef CONFIG_CMD_RKUSB
	printf("enable_endpoint bulk_in cmd rkusb\n");
	d = fsg_ep_desc(common->gadget,
			&fsg_fs_bulk_out_desc, &fsg_hs_bulk_out_desc,
			&fsg_ss_bulk_out_desc, &fsg_ss_bulk_out_comp_desc,
			fsg->bulk_out);
	#else
	printf("enable_endpoint bulk_in\n");
	d = fsg_ep_desc(common->gadget,
	...
	#ifdef CONFIG_CMD_RKUSB
	printf("enable_endpoint bulk_out cmd rkusb\n");
	d = fsg_ep_desc(common->gadget,
			&fsg_fs_bulk_out_desc, &fsg_hs_bulk_out_desc,
			&fsg_ss_bulk_out_desc, &fsg_ss_bulk_out_comp_desc,
			fsg->bulk_out);
	#else
	printf("enable_endpoint bulk_out\n");
	d = fsg_ep_desc(common->gadget,
			&fsg_fs_bulk_out_desc, &fsg_hs_bulk_out_desc);
	#endif
...
static int fsg_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if (IS_RKUSB_UMS_DNL(c->cdev->driver->name)) {
		printf("usb_copy_descriptors rkusb_fs_function\n");
		for (i = 0; rkusb_fs_function[i]; i++) {
			printf("rkusb_fs_function[%d]: %p\n", i, rkusb_fs_function[i]);
		}
		f->descriptors = usb_copy_descriptors(rkusb_fs_function);
	}
	else {
		printf("usb_copy_descriptors fsg_fs_function\n");
		for (i = 0; fsg_fs_function[i]; i++) {
			printf("fsg_fs_function[%d]: %p\n", i, fsg_fs_function[i]);
		}
	...
	if (gadget_is_dualspeed(gadget)) {
		...
		if (IS_RKUSB_UMS_DNL(c->cdev->driver->name)) {
			printf("usb_copy_descriptors rkusb_hs_function\n");
			for (i = 0; rkusb_hs_function[i]; i++) {
				printf("rkusb_hs_function[%d]: %p\n", i, rkusb_hs_function[i]);
			}
			f->hs_descriptors =
				usb_copy_descriptors(rkusb_hs_function);
		}
		else {
			printf("usb_copy_descriptors fsg_hs_function\n");
			for (i = 0; fsg_hs_function[i]; i++) {
				printf("fsg_hs_function[%d]: %p\n", i, fsg_hs_function[i]);
			}
			f->hs_descriptors =
